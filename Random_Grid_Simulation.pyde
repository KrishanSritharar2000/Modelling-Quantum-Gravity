...
def grid(col, row, len_th, mode, coordinates):
    ...
    if mode.lower() == "random":
        for i in range(col*row):
            ...
            strokeWeight(2)
            point(x,y)
            globals()["PVector%s" % (i+1)] = PVector(x,y)
...
def find_nearest_point(find_x,find_y,col,row):
    temp_vector = PVector(find_x,find_y)
    results = []
    for i in range(col*row):
        Pvect = ("PVector%s" % (i+1))
        Pvect_temp = Pvect.copy()
        print(Pvect,"\t",Pvect_temp)
        # distance = temp_vector.dist(Pvect)
...
def drawLinesRandom(walk_num, step_num, col, row, len_th, begin, coord, Pright, Pleft, Pdown, Pup):
    ...
    xpos = x
    y = (displayHeight/2)
    find_nearest_point(xpos,y,col,row)
